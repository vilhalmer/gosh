...
fn main() {
    ...
    let random_fact = Range::new(0, numFacts - 1);

    //List of potato facts
    let facts =  ["Today potatoes are grown in all 50 states of the USA and in about 125 countries throughout the world.",
	...
	println!("\nHere are {:?} potato facts!:\n", args[1].parse::<i8>().unwrap());

   	//print I number of potato facts
	for _ in 0..args[1].parse::<i8>().unwrap() {
		let toPrint = random_fact.ind_sample(&mut rng);
